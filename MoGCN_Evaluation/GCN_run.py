...
def train(epoch, optimizer, features, adj, labels, idx_train):
    ...
    if (epoch + 1) % 10 == 0:
        print(
            "Epoch: %.2f | loss train: %.4f | acc train: %.4f"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--featuredata", "-fd", type=str, required=True, help="The vector feature file."
    )
    parser.add_argument(
        "--adjdata", "-ad", type=str, required=True, help="The adjacency matrix file."
    )
    parser.add_argument(
        "--labeldata", "-ld", type=str, required=True, help="The sample label file."
    )
    ...
    parser.add_argument(
        "--epochs", "-e", type=int, default=150, help="Training epochs, default: 150."
    )
    parser.add_argument(
        "--learningrate",
        "-lr",
    ...
    parser.add_argument(
        "--nclass", "-nc", type=int, default=4, help="Number of classes, default: 4"
    )
    ...
    # load input files
    adj, data, label = load_data(
        args.adjdata, args.featuredata, args.labeldata, args.threshold
    )
    ...
    if args.mode == 0:
        ...
        for idx_train, idx_test in skf.split(data.iloc[:, 1:], label.iloc[:, 1]):
            ...
            idx_train, idx_test = torch.tensor(
                idx_train, dtype=torch.long, device=device
        ...
        GCN_model.to(device)
        optimizer = torch.optim.Adam(
            GCN_model.parameters(), lr=args.learningrate, weight_decay=args.weight_decay
        )
        idx_train, idx_test = torch.tensor(
        ...
        """
        Modified code by Davide Greco to implement only training
        """

        all_sample = data["Sample"].tolist()
        train_sample = list(set(all_sample))
        ...
        idx_train = torch.tensor(train_idx, dtype=torch.long, device=device)
    ...
    if args.mode == 3:
        ...
        all_sample = data["Sample"].tolist()
        test_sample = list(set(all_sample))

        idx_test = data[data["Sample"].isin(test_sample)].index.tolist()
        ...
        GCN_model.load_state_dict(torch.load("model/GCN/GCN_model.pkl"))
        GCN_model.to(device)
